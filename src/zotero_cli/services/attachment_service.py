    ...
    def attach(
        ...
        *,
        ...
        if self._backend == "webdav":
            return self._attach_webdav(parent_key, file_path, reuse_key=reuse_key, force=force)
    ...
    def _attach_webdav(
        ...
        *,
        ...
        result = upload_attachment_webdav(
            storage, parent_key, file_path, self._api,
            reuse_key=reuse_key, force=force,
        )
...
def _format_zfs_result(
    result: dict[str, Any],
    file_name: str, parent_key: str, file_size: int,
) -> Any:
    ...
    for s in result.get("success", result.get("successful", [])):
        ...
        uploaded.append({
            "file": file_name, "attachment_key": str(key),
            "parent_item_key": parent_key,
            "size_bytes": file_size, "md5": "",
            "version": 0, "webdav_path": None, "mtime_ms": None,
        })
        if key:
    ...
    for u in result.get("unchanged", []):
        ...
        unchanged.append({
            ...
            "parent_item_key": parent_key,
            "size_bytes": file_size, "md5": "",
            "version": None, "webdav_path": None, "mtime_ms": None,
        })
    ...
    return {
        "data": {"backend": "zfs", "uploaded": uploaded, "unchanged": unchanged, "failed": failed},
        "meta_extra": {"affected_keys": affected, "backend": "zfs"},
    }


def _format_webdav_result(result: dict[str, Any]) -> Any:
    affected = [u.get("attachment_key", "") for u in result.get("uploaded", [])]
    return {
        "data": result,
...
def _merge_attach_result(
    ...
    all_uploaded: list[Any], all_unchanged: list[Any],
    all_failed: list[Any], affected: list[str],
) -> None:
    d = r["data"]
